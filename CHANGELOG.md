...
- `meta` metadata field on `Network` and `NetworkIXLan` (#1751)
- netixlan `status` value `not-operational`; existing `operational=False` rows are migrated from the boolean. Local queries filtering on `status="ok"` stop returning non-operational connections once the migration has run -- use `status__in=["ok", "not-operational"]` to keep the previous result set (#1742)
### Fixed
...
### Changed
- Expanded model field help text for API semantics, including units, formats, defaults and read-only behavior (peeringdb/peeringdb#1981)
...
### Deprecated
- writing `NetworkIXLan.operational` — it is derived from `status` and becomes read-only at the API layer; the field will be dropped from the schema in a later release (#1742)
### Removed
